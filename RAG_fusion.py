...
QDRANT_COLLECTION_NAME = "ACTRESS_DRESS_TEXT"
...
def dress_retriever(query: str) -> list[Document]:
    """RRF retriever
...
["",""]
    # Example usage
    directory = 'doc_text'  # Directory containing the .txt files
    documents = read_txt_files(directory)
    upload_chunks_to_QDrant(documents)
    
    query = """
I am going to a marriage in Mumbai. Which dress will be suitable for me?
...
- Eye Colour: Green
- Hair Colour: Blonde
- Body Type: Classic hourglass figure, balanced shoulder-to-hip ratio, naturally defined waist.
- Style Preference: Glamorous red carpet gowns, casual chic outfits, form-fitting dresses, tailored jumpsuits, sleek modern attire that enhances curves. Confident persona with a sharp sense of style, embracing outfits that highlight her well-proportioned silhouette.

    
   
    retrieved_documents = rrf_retriever(query, directory)
